...
```{r}
ggplot(mtcars, aes(x = mpg, y = wt, color = cyl)) + geom_smooth() + geom_point()
```

4) Remove the error shading from the smooth line by changing the setting of se to FALSE

```{r}
ggplot(mtcars, aes(x = mpg, y = wt, color = cyl)) + geom_smooth(se = FALSE) + geom_point()
```

5) make the points 60% transparent or 40% visible by setting the alpha to 0.4

```{r}
ggplot(mtcars, aes(x = mpg, y = wt, color = cyl)) + geom_smooth(se = FALSE) + geom_point(alpha = 0.4)
```

6) lastly set the method to lm, to make this a linear line and set the linetype to 2 for a dotted line.

```{r}
ggplot(mtcars, aes(x = mpg, y = wt, color = cyl)) + geom_smooth(method = lm, linetype = 2, se = FALSE) + geom_point(alpha = 0.4)
```
...
```{r}
bakeoff %>%
  filter(showstopper == "" || showstopper == "NA")
```
...
gapminder %>%
filter(year == 2002) %>%
mutate(lifeExpec=12 * lifeExp) %>%
arrange(desc(lifeExpec))
```
...
```{r}
ggplot(gapminder, aes(x=lifeExp, y=gdpPercap,size=pop, color=continent))+geom_point() + facet_wrap("year")
```
